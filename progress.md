...
- [x] **`randomize_letters` feature added to PushText-v1** — At episode start, 2 letters are sampled from a configurable pool (`letter_pool=` kwarg). All pool tiles are pre-built at scene load; inactive ones are parked at z=-100 each episode. Enabled via `--env-kwargs "randomize_letters=true,letter_pool=ABCDEFGH"`. Keep pool ≤ 8 letters to avoid excess actors (each pool letter adds 2 actors per env; with 4096 envs this scales quickly). Tests in `tests/test_push_text.py::TestRandomizeLetters`.
